...
def parse_arguments(cli_args: List[str] = None) -> Namespace:
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="The file path to update, relative to repository root e.g. file.txt",
    )
    parser.add_argument(
        "--content", type=str, required=True, help="Content to add to file"
    )
    parser.add_argument(
        "--user", type=str, required=True, help="User who made the change"
    )
    parser.add_argument(
        "--diff-only",
        action="store_true",
        help="Produce only git diff if on, commit otherwise",
    )
